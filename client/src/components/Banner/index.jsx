...
const Banner = (props) => {
    ...
    return (
        ...
        <div className="perspective-[2000px] w-full ">
            ...
            className="relative w-full h-full min-h-[500px] bg-[#0a0a0a] rounded-3xl overflow-hidden group cursor-none shadow-[0_20px_50px_rgba(15,23,42,0.15)] hover:shadow-[0_25px_60px_rgba(15,23,42,0.22)] border-t border-l border-white/20 border-r border-b border-slate-300/40 transition-all duration-500"
            >
            {/* LAYER 0: Deep Background */}
            <motion.div style={{ x: bgX, y: bgY, scale: 1.15 }} className="absolute inset-0 z-0">
                {/* Heavily Blurred Ambient Image Background to match project aesthetic */}
                {item?.images?.[0] ? (
                    <div className="absolute inset-0 w-full h-full z-0 overflow-hidden pointer-events-none">
                        <img 
                            src={item.images[0]} 
                            className="w-full h-full object-cover filter blur-[50px] opacity-[0.35]" 
                            alt="" 
                        />
                ...
                className="absolute top-0 left-0 w-96 h-96 bg-primary/15 rounded-full blur-[120px]"
                />
                <motion.div
                animate={{ 
                    x: [0, -80, 0], 
                ...
                className="absolute bottom-0 right-0 w-96 h-96 bg-orange-400/10 rounded-full blur-[120px]"
                />
            </motion.div>
            ...
            <motion.div style={{ x: midX, y: midY }} className="absolute inset-0 z-10 pointer-events-none">
                ...
                >
                    {Array(10).fill("HYPERLINE • DIGITAL ERA • KWANGYA ARCHIVE").map((text, i) => (
            ...
            >
                ...
                <div className="space-y-3 sm:space-y-6">
                    <div className="space-y-1 sm:space-y-2">
                        <h3 className="text-white text-[10px] sm:text-xs font-black tracking-[0.3em] uppercase">THE ART OF DUALITY</h3>
                        <h2 className="text-white text-[clamp(2.5rem,8vw,4rem)] font-black leading-[0.9] tracking-tighter">
                        {item?.bannerTitle || "UNBOUND"}
                        </h2>
                    </div>
                </div>
            </motion.div>
            ...
            {/* Border Highlight */}
            <div className="absolute inset-0 rounded-3xl border border-white/10 pointer-events-none z-[120]" />
            </motion.div>
        </div>
        </div>
    );
