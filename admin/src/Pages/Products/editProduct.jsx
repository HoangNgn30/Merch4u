...
const EditProduct = () => {
    ...
    useEffect(() => {
        ...
        fetchDataFromApi(`/api/product/${context?.isOpenFullScreenPanel?.id}`).then((res) => {
            ...
            const clothingSet = ['S', 'M', 'L', 'XL', 'XXL'];
            if (sizes.length === 0) {
                setVariantType('none');
            } else if (sizes.some(s => clothingSet.includes(s))) {
                setVariantType('clothing');
                setSelectedClothingSizes(sizes);
            } else {
                setVariantType('other');
                setVersionCount(sizes.length);
                setVersionNames(sizes);
            }
    ...
    const handleSubmitg = (e) => {
        ...
        if (formFields?.countInStock === "") {
            context.alertBox("error", "Vui lòng nhập số lượng hàng tồn kho");
            return false;
        }
        ...
        if (formFields?.discount === "") {
            context.alertBox("error", "Vui lòng nhập mã giảm giá sản phẩm");
            return false;
        }
            ...
            <form className='form py-1 p-1 md:p-8 md:py-1' onSubmit={handleSubmitg}>
                <div className='scroll max-h-[72vh] overflow-y-scroll pr-4'>
                    ...
                    <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 mb-3 gap-4'>
                        ...
                        <div className='col'>
                            <h3 className='text-[14px] font-[500] mb-1 text-black'>Danh mục phụ cấp ba sản phẩm</h3>
                    ...
                    <div className='col w-full p-5 px-0'>
                        <h3 className="font-[700] text-[18px] mb-3">Ảnh</h3>
